...
static const char DRIVER_PROP_AP[]    = "wlan.ap.start";
...
int wifi_start_supplicant(int p2p_supported)
{
    ...
    int count = 2000; /* wait at most 20 seconds for completion */ //dg change 200 to 2000 for enough times to start wpa_supplicant
#ifdef HAVE_LIBC_SYSTEM_PROPERTIES
    const prop_info *pi;
    unsigned serial = 0, i;
#endif
...
#if 0
    //add by dg for wifi
    property_set("wlan.p2p.status", "ok");
    usleep(1000000);
#endif
...
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
    int count = 5000; /* wait at most 5 seconds for completion */ //dg change 50 to 5000 for enough times to stop woa_supplicant.it is important .
...
int wifi_change_fw_path(const char *fwpath)
{
    ...
    if (strcmp(fwpath, "P2P") == 0)
    {
         ALOGD("break P2P func,add by dg for debug------ ");
    ...
    {
         ALOGD("set property %s ,add by dg for debug------ ",DRIVER_PROP_AP);

         property_set(DRIVER_PROP_AP,"ok");

         return 0;

    }

 #if 0 //dg add for debug
    fd = TEMP_FAILURE_RETRY(open(WIFI_DRIVER_FW_PATH_PARAM, O_WRONLY));
    if (fd < 0) {
        ALOGE("Failed to open wlan fw path param:%s----%s--- (%s)",WIFI_DRIVER_FW_PATH_PARAM,fwpath, strerror(errno));
        return -1;
    }


   ALOGD("Success to open wlan fw path param:%s----%s---",WIFI_DRIVER_FW_PATH_PARAM,fwpath);
